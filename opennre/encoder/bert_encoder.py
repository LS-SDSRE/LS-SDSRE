...
class BERTEncoder(nn.Module):
    ...
    def tokenize(self, item):
        ...
        return id, indexed_tokens, att_mask, pos1, pos2
...
class BERTEntityEncoder(nn.Module):
    def __init__(self, max_length, pretrain_path, blank_padding=True, mask_entity=False):
        ...
        self.bert = BertModel.from_pretrained(pretrain_path)
        self.tokenizer = BertTokenizer.from_pretrained(pretrain_path)
        # self.linear = nn.Linear(self.hidden_size, 128)
    ...
    def get_cls_emb(self, token, att_mask):
        _, x = self.bert(token, attention_mask=att_mask, return_dict=False)
        return x
    ...
    def tokenize(self, item):
        ...
        pos1_mask = torch.zeros(indexed_tokens.size()).long()
        pos2_mask = torch.zeros(indexed_tokens.size()).long()
        pos1_mask[5:7]=1

        if not rev:
            pos1_mask[0, (1 + len(sent0)): (1 + len(sent0+ent0))] = 1
            pos2_mask[0, (1 + len(sent0 + ent0 + sent1)): (1+len(sent0 + ent0 + sent1+ent1))] = 1
        else:
            pos1_mask[0, (1 + len(sent0 + ent0 + sent1)): (1 + len(sent0 + ent0 + sent1+ent1))] = 1
            pos2_mask[0, (1 + len(sent0)): (1 + len(sent0+ent0))] = 1
        return id, indexed_tokens, att_mask, pos1, pos2, pos1_mask, pos2_mask
