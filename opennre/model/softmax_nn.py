...
class SoftmaxNN(SentenceRE):
    ...
    def __init__(self, sentence_encoder, num_class, rel2id):
        ...
        # self.fc = nn.Linear(self.sentence_encoder.hidden_size, num_class-1)
        self.linear = nn.Linear(self.sentence_encoder.hidden_size, self.sentence_encoder.hidden_size)
        self.fc = nn.Linear(self.sentence_encoder.hidden_size, num_class - 1)
    ...
    def forward(self, *args):
        ...
        rep = self.linear(rep)
        logits = self.fc(self.drop(rep)) # (B, N)
        return logits, rep
