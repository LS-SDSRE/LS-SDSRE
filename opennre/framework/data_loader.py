...
class SentenceREDataset(data.Dataset):
    ...
    def __init__(self, path, rel2id, tokenizer, kwargs):
        ...
        self.kwargs = kwargs
        self.neg_id=12
        self.class_weight_np = torch.ones(len(self.rel2id) - 1, dtype=torch.float32)

        self.class_weight = torch.ones(len(self.rel2id), dtype=torch.float32)
        self.id2reltoken = {}
        self.token = BertTokenizer.from_pretrained("/home/18031110270/OpenNRE_our_CE_KL/pretrain/bert-large-uncased")

        for k in self.rel2id.keys():
            self.id2reltoken[self.rel2id[k]] = torch.Tensor(self.token(k.replace("/", " ").replace("_", " ")).input_ids[1:-1]).long()
        ...
        for line in f.readlines():
            ...
            if len(line) > 0:
                l = eval(line)
                l['id'] = i
                label_sentence.append(self.rel2id[l['relation']])
                i+=1
                self.data.append(l)
                # self.data.append(eval(line))
        f.close()
        stat = Counter(label_sentence)
        for k, v in stat.items():
            if k != self.neg_id:
                self.class_weight[k] = 1. / v ** 0.05
                self.class_weight_np[k] = 1. / v ** 0.05
            else:
                self.class_weight[k] = 1. / v ** 0.05
        if torch.cuda.is_available():
            self.class_weight = self.class_weight.cuda()
            self.class_weight_np = self.class_weight_np.cuda()
    ...
    def eval(self, pred_result, use_name=False):
        ...
        pred_positive = 0
        gold_positive = 0
        correct_positive_label = [0]*(len(self.rel2id)-1)
        pred_positive_label = [0]*(len(self.rel2id)-1)
        gold_positive_label = [0]*(len(self.rel2id)-1)
        neg = -1
        for name in ['NA', 'na', 'no_relation', 'Other', 'Others', 'None']:
        ...
        for i in range(total):
            ...
            if golden == pred_result[i]:
                correct += 1
                if golden != neg:
                    correct_positive_label[golden] += 1
                    correct_positive += 1
            if golden != neg:
                gold_positive_label[golden] += 1
                gold_positive +=1
            if pred_result[i] != neg:
                # print(pred_positive_label)
                # print(pred_result[i])
                pred_positive_label[pred_result[i]] += 1
                pred_positive += 1
        ...
        r_label = []
        p_label = []
        for i in range(len(self.rel2id)-1):
            try:
                p_label.append(float(correct_positive_label[i])/float(pred_positive_label[i]))
            except:
                p_label.append(0)
            try:
                r_label.append(float(correct_positive_label[i])/float(gold_positive_label[i]))
            except:
                r_label.append(0)
        p=0
        r=0
        for i in range(len(p_label)):
            p += p_label[i]
            r += r_label[i]
        macro_p = float(p) / float(len(p_label))
        macro_r = float(r) / float(len(r_label))
    ...
    def loss_weight_np(self):
        return self.class_weight_np
